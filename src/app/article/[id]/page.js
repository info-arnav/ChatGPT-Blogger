...
export default async function Article({ params }) {
  ...
  if (articles.data.articles.length == 0) {
    await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "user",
            content: `Write a blog for a blogging website being really descriptive as to compete with the content on wikipdeia in word count greater than 1500 with the title '${params.id}', enclode the content in a htl article tag with appropriate break line tags. dont add title html or body tags. add bold and underline where necessary. and focus on any kewords you find appropriate for the topic. just give me the article without and other introductory sentence`,
          },
        ],
      }),
    })
        ...
        async (res) =>
          await fetch(process.env.GRAPHQL_STRING, {
            ...
            body: JSON.stringify({
              query: `
      mutation {
        insertOneArticle (data : {title:"${params.id.toLowerCase()}", description:"${
    ...
    `,
      ...
      .catch((e) => console.log("Some error while requesting article"));
  }
  return (
    <>
